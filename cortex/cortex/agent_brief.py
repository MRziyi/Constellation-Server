...
def build_agent_brief(
    *,
    ...
    # 3. Rules — YOU MUST emphasis. R3 (Zack 2026-06-02): people/projects resolve
    # from the Twin FIRST + spoken names are STT-fuzzy, so match on alias not literal.
    L.append("== RULES ==")
    ...
    L.append("R3  PEOPLE & PROJECTS resolve from the Twin FIRST (铁律). When the ask names a")
    L.append("    PERSON, YOU MUST grep ~/constellation/twin/people/ and read their file")
    L.append("    BEFORE acting (mail search, drafting, reminding…) — it has their real name")
    L.append("    + email/identity. Spoken names arrive via STT and are OFTEN MIS-HEARD")
    L.append("    ('Jackie'→'Jacky'/'Jackie Tenth'/'Jacy'): match PHONETICALLY / fuzzily on")
    L.append("    name AND aliases — never trust the literal spoken string, never search")
    L.append("    Mail/Contacts with the raw token. A named PROJECT → read projects/<Name>.md")
    L.append("    first for its repos/context.")
    L.append("")
    ...
    L.append("== ZACK'S TWIN  (~/constellation/twin/  — this is your CWD; relative paths land here) ==")
    L.append("Zack's personal knowledge base. **RESOLVE FROM THE TWIN FIRST**: when the ask")
    L.append("is unclear, or Zack names a PERSON or a PROJECT, READ the relevant twin file")
    L.append("BEFORE guessing or asking — the answer is almost always already here.")
    L.append("Layout (see ~/constellation/twin/README.md for the full contract):")
    L.append("  projects/<Name>.md      one file per project — what it is + its CODE PATHS. READ THIS")
    L.append("                          FIRST whenever Zack names a project (e.g. 'Chrono'): it points")
    L.append("                          you at the repos so you never have to hunt for them.")
    L.append("  people/core/<slug>.md   close people · people/encounters/<slug>.md  people he's met.")
    L.append("                          minimal frontmatter (aliases / relation / email), prose body.")
    L.append("                          READ FIRST when he names someone (match on name OR alias).")
    L.append("  identity.md             Zack himself (name, accounts, voice, taste). Read when relevant.")
    L.append("  memos/<slug>.md         his saved notes (often with an embedded photo).")
    L.append("  follow-ups.md           commitments he made to people (what he owes + by when).")
    L.append("                          READ/APPEND for 'note I'll follow up with X' + 'what do I")
    L.append("                          owe people' — see the conference-followups skill.")
    L.append("  conferences/<slug>/     program.json (talks/posters w/ ISO start/end + room) + meta.json.")
    L.append("                          For 'plan my next hour' / 'sessions like this I'd care about'")
    L.append("                          — FILTER by time window first (it's big), see conference-program skill.")
    L.append("  receipts/<date>.md      Cortex's action log (you don't need to read these).")
    L.append("  .claude/skills/<n>/SKILL.md  auto-discovered by you (email-style / reminder-style / code-style).")
    L.append("")
    ...
    L.append("  new memo → memos/<slug>.md · new project note → projects/<Name>.md.")
    L.append("  Don't invent top-level dirs. Don't seed placeholders.")
    L.append("")
    L.append("Read on demand the file that matters — don't preemptively scan the whole Twin.")
    L.append("")
    ...
    L.append("  end tell")
    L.append("")
    L.append("MAIL — answer about CONTENT, not just metadata: when Zack asks what an")
    L.append("email SAYS / what someone is ASKING, you MUST read the BODY (`content of m`)")
    L.append("and tell him what it actually says. Date/subject/sender alone is NOT an")
    L.append("answer — finish the job and report the substance.")
    ...
    L.append("  Then fetch the BODY by ID in a SECOND osascript (`content of m`) — way")
    L.append("  faster than iterating + body-reading in one pass, but always DO it.")
